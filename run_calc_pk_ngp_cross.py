...
# input two file names for the cross-power spectrum [need more thoughts how to name them properly; right now just using the same file]
	Poissonfname1 = 'lognormal/lognormal_is'+str(iseed)+'_rlz'+str(i)+'_Pnmax'+str(Pnmax)+'_b'+str(bias)+'.bin'	#output lognormal galaxy catalog in (x,y,z,vx,vy,vz) [h^-1 Mpc] and [km/s]
	Poissonfname2 = 'lognormal/lognormal_is'+str(iseed)+'_rlz'+str(i)+'_Pnmax'+str(Pnmax)+'_b'+str(bias)+'.bin'	#output lognormal galaxy catalog in (x,y,z,vx,vy,vz) [h^-1 Mpc] and [km/s]
        pkfname='cross_pk/cross_pk_ngp_is'+str(iseed)+'_rlz'+str(i)+'_z'+str(zz)+'_b'+str(bias)+'_los'+str(losx)+'.'+str(losy)+'.'+str(losz)+'.dat'
        executable('calculate_cross/calc_pk_const_los_ngp_cross')([Poissonfname1,Poissonfname2,nmax,aHz,losx,losy,losz,kbin,kmax,lmax,imulfname,pkfname,p2fname])     # same as before, but with two more inputs (lmax and imulfname)
