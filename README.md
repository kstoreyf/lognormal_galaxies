...
- (v5) By Ryu Makiya, Sep 04, 2016.
	...
	-- Includes a new option for the estimation of the power spectrum, in which the Pk is estimated in the cubic box
	   which is large enough to emcompass the whole survey region. (set calc\_mode\_pk = 1 in .ini file to use it)
...
1. The resulting files are as follows:
	...
	For each realization, the code generates the followng two binary files:
		...
		- example\_lognormal\_rlz{# of realization}.bin (for galaxy field)  
		The positions and velocities of galaxies are stored in those files. The larger "Pnmax", the larger the memory that the code requires. Start with a smaller value such as 512 when testing the code.  
		To read them, use a sample code "lognormal/read_lognormal.f90". The byte-by-byte description of the file is:
		...
		- example\_density\_lognormal\_rlz{# of realization}.bin (for density field)			
		The matter density in each cell is store in those files.; The byte-by-byte description of the file is:
